...
class SetCriterionBAST(nn.Module):
    ...
    def _pairwise_loc_cost(pred_loc, gt_loc):
        """
        pred_loc: [K,2], gt_loc: [N,2] in degrees (az, el)
        Returns cost[K,N] = (wrapped azimuth error^2 + elevation error^2)
        """
        if gt_loc.numel() == 0:
        ...
        # Azimuth wrap
        delta_az_rad = (pred[..., 0] - tgt[..., 0]) * (torch.pi / 180.0)
        az_err_deg = torch.atan2(torch.sin(delta_az_rad), torch.cos(delta_az_rad)) * (180.0 / torch.pi)
        el_err = pred[..., 1] - tgt[..., 1]
        return az_err_deg.pow(2) + el_err.pow(2)

    @staticmethod
    def _pairwise_cls_cost(pred_cls_logit, gt_cls_onehot):
        """
        pred_cls_logit: [K,C]
        ...
        bce = -(y_exp * torch.log(p_exp) + (1 - y_exp) * torch.log(1 - p_exp))  # [K,N,C]
        return bce.sum(-1)  # [K,N]
    ...
    def _hungarian(self, pred_loc, pred_obj_logit, pred_cls_logit, gt_loc, gt_cls):
        ...
        N = gt_loc.size(0)
        if N == 0:
            return torch.empty(0, dtype=torch.long, device=pred_loc.device), torch.empty(0, dtype=torch.long, device=pred_loc.device)
        ...
        with torch.no_grad():
            ...
            total_cost = (self.loc_cost_weight * loc_cost +
            ...
            pred_inds = torch.as_tensor(row_ind, dtype=torch.long, device=pred_loc.device)
    ...
    def forward(self, outputs, targets):
        ...
        for b in range(B):
            ...
            if N > 0:
                ...
                pred_idx, gt_idx = self._hungarian(pred_loc_b, pred_obj_b, pred_cls_b, gt_loc, gt_cls)
                ...
                obj_loss_b = F.binary_cross_entropy_with_logits(pred_obj_b, obj_target, reduction='mean')
            else:
